...
class App:
    ...
    def _setup_main_page(self):
        ...
        self._setup_labels()
        Button(self.frame, text='К тратам', command=lambda: self.make_page(self.page1)).grid(row=6, column=0)
        Button(self.frame, text='К прибыли', command=lambda: self.make_page(self.page2)).grid(row=6, column=1)
        Button(self.frame, text='К категориям', command=lambda: self.make_page(self.page3)).grid(row=6, column=2)

    def _setup_labels(self):
        Label(self.frame, textvariable=self.totalVAR, font=self.main_font).grid(row=2, columnspan=4,
                                                                                ipady=30, ipadx=30)
        ...
        Label(f_gains, text="Доходы за месяц:", font=self.sec_font).grid(row=0, column=0)
...
class SpendDialog(tksd.Dialog):
    ...
    def _setup_listbox(self, master):
        ...
        self.e2.config(width=50, height=10)
